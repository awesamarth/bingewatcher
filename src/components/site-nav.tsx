...
export function SiteNav({ active, overlay = false }: { active?: "explore" | "lineups"; overlay?: boolean }) {
  ...
  return (
    <nav className={`fixed top-0 left-0 z-40 flex w-full items-center justify-between px-5 py-5.5 transition-[background-color,backdrop-filter] duration-300 sm:px-[8vw] ${surface}`}>
      <Brand large />
      <div className={`flex items-center gap-4 text-[13px] font-semibold text-white/90 transition-[text-shadow] duration-300 sm:gap-5 sm:text-sm ${textContrast}`}>
        <Link className={active === "explore" ? "text-white" : "hover:text-white"} href="/explore">Explore</Link>
        <Link className="hidden hover:text-white sm:block" href="/explore#watchlist">Watchlist</Link>
