...
- Backend-only TMDB movie/TV search, discover, genre, provider, details, season/episode, trailer, recommendation, and community score/vote-count integration works with `TMDB_READ_API_KEY`. The homepage waits for a runtime request before loading its cached TMDB poster data so Docker builds without runtime secrets cannot bake in an empty hero.
- Explore supports paginated trending/popular shelves, title search combinable with genre/provider/language/year/runtime/region/sort filters, shareable filter URLs, personalized recommendation seeds, watchlist toggles, and direct add-to-lineup with live counts, membership detection, Remove/Add again, and View lineup feedback. Search and filtered-result grids automatically load additional TMDB pages near the bottom only when more pages exist; editorial shelves retain View more/View less. The default watchlist has a dedicated responsive `/watchlist` library with title navigation and removal; it is no longer embedded at the bottom of Explore. Shared cinematic dropdown controls are used across the app.
- Rich movie, whole-series, and season pages support linked regional Stream/Rent/Buy provider offers and logos through the backend-only Streaming Availability API, with cached responses and TMDB provider names as fallback, plus trailers, recommendations, watchlist actions, lineup actions, episode-level progress, bulk season/show completion, and lightweight reactions. Their contextual back action returns to the originating Explore state, watchlist, or lineup; Chromium uses real history-back to preserve scroll, with a validated route fallback elsewhere.
...
- 2026-09-03: Watchlist is a first-class `/watchlist` destination, not an Explore shelf. Global navigation and `open_watchlist` both open that page.
- 2026-09-03: Explore text search preserves and applies active discovery filters. Because TMDB has no combined search/discover endpoint, genre/language/year filters use search metadata and provider/runtime filters enrich each returned search page with cached TMDB details.
- 2026-09-03: Title-page contextual back navigation carries a validated internal return destination from Explore, watchlist, and lineup cards. Use browser history when the preceding entry matches so query state and scroll survive; otherwise navigate to the safe fallback.
- 2026-09-04: Render TMDB images directly from TMDB's already-sized CDN URLs instead of cold-optimizing them on the 0.5 CPU application instance. Homepage TMDB data is runtime-rendered and fetch-cached because Docker build steps do not receive the runtime API token.
